...
public class DruidInitializer implements WebApplicationInitializer {
	...
	public void onStartup(ServletContext servletContext) throws ServletException {
		...
		druidWebStatFilter.setInitParameter("principalSessionName", AppConstants.DRUID_STAT_PRINCIPAL_SESSION_NAME);
		...
		druidWebStatFilter.addMappingForUrlPatterns(
				...
				AppConstants.API_MAPPING_URL_PATTERN);

		// Servlet
		ServletRegistration.Dynamic druidStatView = servletContext.addServlet("DruidStatView", new StatViewServlet());
		druidStatView.setInitParameter(StatViewServlet.PARAM_NAME_USERNAME, AppConstants.DRUID_STATVIEW_USERNAME);
		druidStatView.setInitParameter(StatViewServlet.PARAM_NAME_PASSWORD, AppConstants.DRUID_STATVIEW_PASSWORD);
		druidStatView.setInitParameter("allow", AppConstants.DRUID_STAT_ALLOW_IP); // 指定可以访问统计页面的IP，提高安全性
		...
		druidStatView.addMapping(new String[] { AppConstants.DRUID_MAPPING_URL_PATTERN });
	}
