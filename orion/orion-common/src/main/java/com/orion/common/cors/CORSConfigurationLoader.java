...
import com.orion.common.cors.environment.Environment;
import com.orion.common.cors.environment.SystemProperties;
...
public class CORSConfigurationLoader {
	 ...
	 * initialisation parameter that points to the file name holding the CORS
	 ...
	 *
	 * @param config
	 *            The filter configuration. Must not be {@code null}.
	 *
	 ...
	 *
	 * @param filename
	 *            The file name. Should begin with a '/' and is interpreted as
	 *            relative to the web application root directory or relative to
	 *            the class path. Must not be {@code null}.
	 * 
	 * @return The properties found in the file.
	 *
	 * @throws IOException
	 *             If the file wasn't found, or the properties couldn't be
	 *             loaded.
	 */
	 ...
	 *
	 * @param env
	 *            The system variables environment.
	 */
	 ...
	 * {@code cors.configurationFile} variable. If it exists, the configuration
	 * properties are loaded from the file location specified by the variable
	 * value. The location must be relative to the web application root
	 * directory or relative to the classpath.
	 * <li>The web.xml filter initialisation parameters are checked for a
	 * {@code cors.configurationFile} variable. If it exists, the configuration
	 * properties are loaded from the file location specified by the parameter
	 * value. The location must be relative to the web application root
	 * directory or relative to the classpath.
	 * <li>The configuration is specified by the web.xml filter initialisation
	 * parameters with the matching name. If an initialisation parameter is not
	 * defined the default configuration property is applied.
	 * </ul>
	 *
	 * @return The loaded CORS filter configuration.
	 *
	 * @throws CORSConfigurationException
	 *             If the configuration file couldn't be loaded or parsing of
