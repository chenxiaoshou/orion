...
public final class AESUtil {
	...
	public static String encryptToBase64(String source) {
		...
		try {
			Cipher cipher = Cipher.getInstance(ALGORITHM);
			cipher.init(Cipher.ENCRYPT_MODE, keySpec);
			result = CodecUtil.toBase64(cipher.doFinal(source.getBytes(AppConstants.CHARSET_UTF_8)));
		} catch (Exception e) {
			LOGGER.error("AES encode failed!", e);
		}
		return result;
	...
	public static String decryptFromBase64(String base64Encrypted) {
		...
		try {
			Cipher cipher = Cipher.getInstance(ALGORITHM);
			cipher.init(Cipher.DECRYPT_MODE, keySpec);
			result = new String(cipher.doFinal(CodecUtil.fromBase64(base64Encrypted)), AppConstants.CHARSET_UTF_8);
		} catch (Exception e) {
			LOGGER.error("AES decode failed!", e);
		}
		return result;
