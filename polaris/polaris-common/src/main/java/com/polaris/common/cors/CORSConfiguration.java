...
/**
 * The CORS filter configuration. The fields become immutable (final) after they
 * are initialised.
 *
...
public class CORSConfiguration {
	...
	/**
	 * If {@code true} generic HTTP requests must be allowed to pass through the
	 * filter, else only valid and accepted CORS requests must be allowed
	 * (strict CORS filtering).
	 *
	 * <p>
	...
	/**
	 * If {@code true} the CORS filter must allow requests from any origin, else
	 * the {@link #allowedOrigins origin whitelist} must be consulted.
	 *
	 * <p>
	 ...
	 * included here must be refused with a HTTP 403 "Forbidden" response. This
	 * property is overridden by {@link #allowAnyOrigin}.
	 *
	 * <p>
	 * Note: The set is of type String instead of Origin to bypass parsing of
	 * the request origins before matching, see
	...
	/**
	 * If {@code true} the CORS filter must allow requests from any origin which
	 * is a subdomain origin of the {@link #allowedOrigins}.
	 *
	 * <p>
	 * Example:
	 *
	 ...
	 * {@code http://foo.example.com}, {@code http://bar.example.com}, etc...
	 *
	 * <p>
	...
	/**
	 * Helper method to check whether requests from the specified origin must be
	 * allowed.
	 *
	 * @param origin
	...
	/**
	 * Helper method to check whether the specified origin is a subdomain origin
	 * of the {@link #allowedOrigins}. This is done by matching the origin's
	 * scheme, hostname and port against each of the {@link #allowedOrigins}.
	 *
	 * <p>
	 * Example:
	 *
	 * <p>
	 * {@code Origin: https://foo.example.com } matches
	 ...
	 *            {@code null} if unknown.
	 *
	 * @return {@code true} if the origin is an allowed subdomain origin, else
	 *         {@code false}.
	 */
	...
	/**
	 * The supported HTTP methods. Requests for methods not included here must
	 * be refused by the CORS filter with a HTTP 405 "Method not allowed"
	 * response.
	 *
	 * <p>
	 ...
	 * This is done by looking up {@link #supportedMethods}.
	 *
	 * @param method
	 *            The HTTP method.
	 *
	...
	/**
	 * If {@code true} the CORS filter must support any requested header, else
	 * the {@link #supportedHeaders supported headers list} must be consulted.
	 *
	 * <p>
	...
	/**
	 * Helper method to check whether the specified (non-simple) author request
	 * header is supported.
	 *
	 * @param header
	...
	/**
	 * The non-simple response headers that the web browser should expose to the
	 * author of the CORS request.
	 *
	 * <p>
	...
	/**
	 * Indicates whether user credentials, such as cookies, HTTP authentication
	 * or client-side certificates, are supported.
	 *
	 * <p>
	...
	/**
	 * Indicates how long the results of a preflight request can be cached by
	 * the web client, in seconds. If {@code -1} unspecified.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The following properties are recognised (if missing they default to the
	 * specified values):
	 *
	 * <ul>
	 ...
	 *
	 * @param props
	 *            The properties. Must not be {@code null}.
	 *
	 * @throws CORSConfigurationException
	 *             On a invalid property.
	 */
	public CORSConfiguration(final Properties props) throws CORSConfigurationException {
		...
		try {
			...
			String methodSpec = pr.getOptString("cors.supportedMethods", "GET, POST, HEAD, OPTIONS").trim()
					.toUpperCase();
				...
				for (String header : headers) {
					...
					try {
						...
						throw new PropertyParseException(
								"Bad header field name in property cors.supportedHeaders: " + header);
					}
				}
			}
			...
			for (String header : parseWords(pr.getOptString("cors.exposedHeaders", ""))) {
				...
				try {
					...
					throw new PropertyParseException(
							"Bad header field name in property cors.exposedHeaders: " + header);
				}
			}
