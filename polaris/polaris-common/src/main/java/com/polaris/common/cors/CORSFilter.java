...
public class CORSFilter implements Filter {
	...
	/**
	 * Creates a new uninitialised CORS filter. Must be then initialised with
	 * {@link #setConfiguration} or {@link #init}.
	 */
	public CORSFilter() {
	...
	/**
	 * Allows an alternative means of setting the configuration that suits a
	 * Spring bean constructor approach rather than a servlet init approach.
	 *
	 * See https://bitbucket.org/thetransactioncompany/cors-filter/issue/24
	 *
	 * @param config
	 *            The cross-origin access policy. Must not be {@code null}.
	 */
	 ...
	 *
	 * @return The cross-origin access policy, {@code null} if the filter is not
	 *         initialised.
	 */
	...
	/**
	 * This method is invoked by the servlet container to initialise the filter
	 * at startup.
	 *
	 * @param filterConfig
	 ...
	 *
	 * @param corsException
	 *            The CORS exception. Must not be {@code null}.
	 * @param response
	 *            The HTTP servlet response. Must not be {@code null}.
	 *
	 * @throws IOException
	 *             On a I/O exception.
	 * @throws ServletException
	...
	private void printMessage(final CORSException corsException, final HttpServletResponse response)
	 ...
	 * @throws ServletException
	 */
	private void returnJsonMessage(final CORSException corsException, final HttpServletResponse response)
			throws IOException, ServletException {
		...
		PrintWriter out = response.getWriter();
		String exceptionMsg = "Cross-Origin Resource Sharing (CORS) Filter: " + corsException.getMessage();
		String message = messageSource.getMessage("global.cors.exception",
				new Object[] { corsException.getHTTPStatusCode(), exceptionMsg }, null);
	...
	private void doFilter(final HttpServletRequest request, final HttpServletResponse response, final FilterChain chain)
		...
		try {
			if (type.equals(CORSRequestType.ACTUAL)) {
				...
				returnJsonMessage(CORSException.GENERIC_HTTP_NOT_ALLOWED, response);
			}
		} catch (CORSException e) {

			returnJsonMessage(e, response);
		}
	}
