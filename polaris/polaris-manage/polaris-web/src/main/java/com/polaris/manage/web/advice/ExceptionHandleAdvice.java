...
public class ExceptionHandleAdvice {
	...
	/**
	 *  FIXME　传入的数据不合格时，并没有起到拦截的作用,需要debug
	 *  
	 *  处理数据绑定时不符合JSR303验证的异常
	 * @param e
	 * @return
	 */
	...
	public ResponseEntity<AppMessage> handleApiException(ApiException e) {
		...
		if (StringUtils.isBlank(message)) {
			message = messageSource.getMessage(ExceptionConstants.APPLICATION_EXCEPTION, null, null);
		}
	...
	public ResponseEntity<AppMessage> handleHttpRequestMethodNotSupportedException(HttpRequestMethodNotSupportedException e) {
	...
	public ResponseEntity<AppMessage> handleHttpMediaTypeNotSupportedException(Exception e) {
