...
public class MockOrderControllerTest extends MockBaseTest {
	...
	/**
	 * Mockito测试风格，必须[mockMvc = MockMvcBuilders.standaloneSetup(orderController).build();]这种方式获取mockMvc
	 * @throws Exception
	 */
	@Test
	public void findOrder() throws Exception {
		...
		when(mockOrderService.findOne(orderId)).thenReturn(order);
		mockMvc.perform(get("/order/{orderId}", orderId).accept(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andDo(print())
		...
		verify(mockOrderService, times(1)).findOne(orderId); // 验证方法是否被mock且是否为所执行的参数调用1次
		verify(mockOrderService, never()).deleteOrder(order);
	}
